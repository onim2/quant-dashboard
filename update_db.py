...
def get_market_data(target_date_str, market_name):
    ...
    # ── 컬럼 타입을 DB 스키마에 맞게 변환 ──────────────────────────────

    # date: datetime
    final['date'] = pd.to_datetime(final['date'])

    # open, high, low, close: kospi=FLOAT / kosdaq=DOUBLE → 둘 다 float으로 OK
    for col in ['open', 'high', 'low', 'close']:
        final[col] = pd.to_numeric(final[col], errors='coerce').astype(float)

    # volume: BIGINT (소수점 없음)
    final['volume'] = pd.to_numeric(final['volume'], errors='coerce').fillna(0).round(0).astype('int64')

    # change_rate: kospi=FLOAT / kosdaq=DOUBLE → float OK
    ...
    if "kospi" in market_name:
        ...
        for col in ['for_net', 'inst_net', 'ind_net']:
            final[col] = pd.to_numeric(final[col], errors='coerce').fillna(0).round(0).astype('int64')
    else:
        # kosdaq: for_net, inst_net, ind_net → DOUBLE
        for col in ['for_net', 'inst_net', 'ind_net']:
            final[col] = pd.to_numeric(final[col], errors='coerce').fillna(0).astype(float)
